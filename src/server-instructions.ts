...
export function buildServerInstructions(mode: InstructionMode = "stdio"): string {
  ...
  const lines = [
    ...
    "- When readyToSell, set priceCents (USD cents, min 100) via beecargo_update_share_settings. Buyers pay on the human /d/{shortId} page.",
    "- Priced machine downloads require a purchaseToken from the share-page claim flow (or owner auth).",
    "",
    "Best practices:",
    ...
    "- Agent policy: if unlockRequired, do not call beecargo_get_download_url without unlock credentials. If payment required, send the human to /d/{shortId} to pay.",
